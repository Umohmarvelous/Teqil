...
const cacheSet = async (key: string, val: unknown) => {
  try { await AsyncStorage.setItem(key, JSON.stringify(val)); } catch { /* ignore */ }
};

// ─── Geocode a text address → LatLng ─────────────────────────────────────────
...
// ─── AnimatedPressable ────────────────────────────────────────────────────────
interface AnimPressProps { onPress: () => void; style?: object; children: React.ReactNode; disabled?: boolean; scaleValue?: number }
function AnimatedPressable({ onPress, style, children, disabled, scaleValue = 0.94 }: AnimPressProps) {
  const scale = useSharedValue(1);
  const animStyle = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  return (
...
function EarningsCounter({ coins }: { coins: number }) {
  const scale = useSharedValue(1);
  useEffect(() => { scale.value = withSequence(withTiming(1.2, { duration: 120 }), withSpring(1, { damping: 12 })); }, [coins]);
  const style = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  return (
    <Animated.View style={[styles.earningsPill, style]}>
      <Ionicons name="star" size={14} color={Colors.gold} />
...
function LiveBadge() {
  const opacity = useSharedValue(1);
  useEffect(() => {
    opacity.value = withRepeat(withSequence(withTiming(0.3, { duration: 800 }), withTiming(1, { duration: 800 })), -1, false);
  }, []);
  const dotStyle = useAnimatedStyle(() => ({ opacity: opacity.value }));
  return (
    <View style={styles.liveBadge}>
...
// ─── SOS Modal ────────────────────────────────────────────────────────────────
function SOSModal({ visible, onConfirm, onCancel }: { visible: boolean; onConfirm: () => void; onCancel: () => void }) {
  const tY = useSharedValue(300); const op = useSharedValue(0);
  useEffect(() => {
    if (visible) { op.value = withTiming(1, { duration: 200 }); tY.value = withSpring(0, { damping: 25 }); }
    else { op.value = withTiming(0, { duration: 150 }); tY.value = withTiming(150, { duration: 200 }); }
  }, [visible]);
  ...
  return (
    <Modal transparent visible={visible} animationType="none">
      <Animated.View style={[styles.modalOverlay, overlayStyle, { paddingBottom: 250 }]}>
        <Animated.View style={[styles.sosSheet, sheetStyle]}>
          <View style={styles.sosIconRing}><Ionicons name="warning" size={36} color={Colors.error} /></View>
          <Text style={styles.sosTitle}>Emergency SOS</Text>
          <Text style={styles.sosDesc}>This will immediately alert your emergency contacts and park owner with your live location.</Text>
          <View style={styles.sosActions}>
            <AnimatedPressable onPress={onCancel} style={styles.sosCancelBtn}>
              <View style={styles.sosCancelInner}><Text style={styles.sosCancelText}>Cancel</Text></View>
            </AnimatedPressable>
...
function PassengerChip({ passenger, index }: { passenger: Passenger; index: number }) {
  ...
  return (
    <View style={styles.passengerChip}>
      ...
      <Text style={styles.passengerDestText} numberOfLines={1}>{passenger.destination || "End"}</Text>
    </View>
  );
}
...
function ChatBubble({ msg, isSpeaking, onSpeak }: { msg: ChatMessage; isSpeaking: boolean; onSpeak: (text: string) => void }) {
  ...
  return (
    <View style={[aiStyles.bubble, isUser ? aiStyles.bubbleUser : aiStyles.bubbleAI]}>
      {!isUser && (
        <View style={aiStyles.bubbleHeader}>
          <View style={aiStyles.aiBubbleIcon}><Ionicons name={(msg.icon as any) ?? "sparkles"} size={13} color={Colors.gold} /></View>
          <Text style={aiStyles.bubbleAILabel}>Teqil AI</Text>
          <Pressable onPress={() => onSpeak(msg.text)} style={aiStyles.speakBtn} hitSlop={8}>
            <Ionicons name={isSpeaking ? "volume-high" : "volume-medium-outline"} size={14} color={isSpeaking ? Colors.primary : "rgba(255,255,255,0.4)"} />
          </Pressable>
      ...
      <Text style={[aiStyles.bubbleText, isUser && aiStyles.bubbleTextUser]}>{msg.text}</Text>
      <Text style={aiStyles.bubbleTime}>{msg.timestamp.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</Text>
    </View>
  );
}

// ─── AI Assistant Modal ───────────────────────────────────────────────────────
function AIAssistantModal({ visible, onClose, ctx }: { visible: boolean; onClose: () => void; ctx: AIContext }) {
  const insets = useSafeAreaInsets();
  const tY = useSharedValue(600); const op = useSharedValue(0);
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [muted, setMuted] = useState(false);
  ...
  useEffect(() => {
    if (visible) {
      op.value = withTiming(1, { duration: 220 }); tY.value = withSpring(0, { damping: 22, stiffness: 200 });
      if (messages.length === 0) {
        const welcome: ChatMessage = {
    ...
    } else { op.value = withTiming(0, { duration: 180 }); tY.value = withTiming(600, { duration: 260 }); }
  }, [visible]);
  ...
  const sendMessage = useCallback(async (question: string) => {
    ...
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setInput(""); stopSpeaking();
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <Animated.View style={[aiStyles.sheet, { paddingBottom: Math.max(insets.bottom, 16) }, sheetStyle]}>
          ...
          <ScrollView ref={scrollRef} style={aiStyles.chatScroll} contentContainerStyle={aiStyles.chatContent} showsVerticalScrollIndicator={false} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: true })}>
            {messages.map(msg => (
              <ChatBubble key={msg.id} msg={msg} isSpeaking={speakingId === msg.id}
                onSpeak={text => { speakingId === msg.id ? stopSpeaking() : speakText(msg.id, text); }} />
            ))}
            {loading && (
              <View style={aiStyles.typingRow}>
                <View style={aiStyles.typingBubble}>
          ...
          <View style={aiStyles.inputRow}>
            <TextInput ref={useRef(null)} style={aiStyles.input} placeholder="Ask me anything about your trip…" placeholderTextColor="rgba(255,255,255,0.3)"
              value={input} onChangeText={setInput} multiline maxLength={200} returnKeyType="send"
              onSubmitEditing={() => sendMessage(input)} editable={!loading} />
...
export default function LiveTripScreen() {
  ...
  const [aiVisible, setAiVisible] = useState(false);
  const [topBarCollapsed, setTopBarCollapsed] = useState(false);
  const [ratingContext, setRatingContext] = useState<null | { tripId: string; ratedUserId: string; raterRole: "driver" | "passenger" }>(null);
  ...
  // ── Geocode origin/dest + fetch route ────────────────────────────────────────
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pts && pts.length > 0) {
        ...
        setRouteError("Add EXPO_PUBLIC_GOOGLE_MAPS_API_KEY to enable route drawing.");
      } else {
        setRouteError("Could not load route. Showing map without directions.");
      }
    })();
  }, [displayTrip?.id]);

  // ── Geocode passenger destinations ──────────────────────────────────────────
  useEffect(() => {
  ...
  // ── Get single location for passenger (driver's last known position) ─────────
  useEffect(() => {
    if (isDriver || !locationPermission) return;
    (async () => {
      try {
  ...
  // ── Center map once we have initial data ─────────────────────────────────────
  useEffect(() => {
  ...
  const aiBtnScale = useSharedValue(1);
  useEffect(() => {
    aiBtnScale.value = withRepeat(withSequence(withTiming(1.06, { duration: 1200 }), withTiming(1, { duration: 1200 })), -1, false);
  }, []);
  const aiBtnStyle = useAnimatedStyle(() => ({ transform: [{ scale: aiBtnScale.value }] }));
    ...
    Alert.alert(t("trip.endTrip"), "This will complete the trip for all passengers.", [
      { text: t("common.cancel"), style: "cancel" },
      {
        ...
        onPress: async () => {
          setIsEnding(true);
          const currentTrip = displayTrip;
          if (trip) await TripsStorage.update(trip.id, { status: "completed", end_time: new Date().toISOString() });
          if (timerRef.current) clearInterval(timerRef.current);
  ...
  // ── SOS ───────────────────────────────────────────────────────────────────────
  const handleSOSPress = () => { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning); setSosVisible(true); };
  const handleSOSConfirm = () => { setSosVisible(false); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error); Alert.alert("SOS Sent", "Emergency contacts and park owner have been notified."); };

  // ── Initial map region ────────────────────────────────────────────────────────
  const initialRegion: Region | undefined = useMemo(() => {
    const center = driverLocation ?? originCoord ?? { latitude: 6.5244, longitude: 3.3792 }; // Lagos fallback
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {routePoints.length > 1 && (
          <Polyline coordinates={routePoints} strokeColor={Colors.primary} strokeWidth={4} lineDashPattern={undefined} />
        )}
      ...
      {/* ── Rating Modal ── */}
      {ratingContext && (
        <RatingModal visible={true}
          onClose={() => { setRatingContext(null); router.replace("/(passenger)"); }}
          tripId={ratingContext.tripId} ratedUserId={ratingContext.ratedUserId} raterRole={ratingContext.raterRole}
          onSubmit={() => { setRatingContext(null); router.replace("/(passenger)"); }} />
      )}
    </View>
  );
...
const aiStyles = StyleSheet.create({
  ...
  overlayTap: { flex: 1 },
  sheet: { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#111A14", borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingTop: 10, maxHeight: "78%", borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.06)", shadowColor: "#000", shadowOffset: { width: 0, height: -8 }, shadowOpacity: 0.5, shadowRadius: 24, elevation: 24 },
