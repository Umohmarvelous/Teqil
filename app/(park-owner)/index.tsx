...
interface DashboardStats {
  ...
  totalEstimatedEarnings: number; // naira
  totalTrips: number;
...
export default function ParkOwnerDashboard() {
  ...
  const load = useCallback(async () => {
    const parkName = (user?.park_name || "").trim().toLowerCase();

    // Fetch all trips; filter those whose driver is in this park.
    // Since we store trips locally without cross-user data, we approximate by
    // matching trip.park_name (if stored) OR showing all trips for now.
    // In production you'd query Supabase. Here we use all local trips as proxy.
    const allTrips = await TripsStorage.getAll();

    // Enrich active trips with passenger count
    ...
    // Estimate coins: 5 per trip + passengers * 2 → convert to naira
    let totalCoins = 0;
    ...
    // Count unique drivers: from trips, count distinct driver_ids
    ...
    >
      ...
      >
        ...
        <View style={styles.parkInfoRow}>
          <View style={styles.parkInfoPill}>
            ...
            <Text style={styles.parkInfoText} numberOfLines={1}>
              {parkLocation}
            </Text>
          </View>
          {stats.activeTrips > 0 && (
      ...
      <View style={styles.statsGrid}>
        <StatTile
          ...
          sub={`${stats.totalTrips} total`}
          accent={Colors.primary}
        />
        <StatTile
          icon="people"
          label={t("parkOwner.totalDrivers")}
          value={stats.totalDrivers > 0 ? stats.totalDrivers.toString() : "—"}
          accent="#3B82F6"
        />
        <StatTile
          icon="checkmark-circle"
          label={t("parkOwner.completionRate")}
          value={stats.totalTrips > 0 ? `${stats.completionRate}%` : "—"}
          sub={stats.completedTrips > 0 ? `${stats.completedTrips} done` : undefined}
          accent={Colors.gold}
        />
        <StatTile
          icon="wallet"
          label="Revenue"
          value={stats.totalEstimatedEarnings > 0 ? formatNaira(stats.totalEstimatedEarnings) : "—"}
          sub="estimated"
          accent="#8B5CF6"
        />
      </View>
...
const styles = StyleSheet.create({
  ...
  tripDivider: {
    ...
    marginLeft: 16 + 10 + 12, // align after dot
  },
  tripDot: {
    width: 10,
    height: 10,
