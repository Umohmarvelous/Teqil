 ...
 * - Bottom action sheet is now a proper PanResponder-driven swipeable modal
 *   triggered by a floating button on the map.
 * - Trip end (driver) / Leave trip (passenger) navigates to /rating screen
 *   with full trip metadata as params.
 * - Push notification scheduled on trip end via notifications service.
 * - Emergency contact SOS uses notifications service.
 */
...
import {
  View, Text, StyleSheet, Pressable, ScrollView, Platform,
  Alert, Dimensions, TextInput, KeyboardAvoidingView, Modal,
  ActivityIndicator, TouchableOpacity, PanResponder, Animated,
} from "react-native";
import { router, useLocalSearchParams } from "expo-router";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { Ionicons } from "@expo/vector-icons";
...
function AnimatedPressable({
  onPress, style, children, disabled, scaleValue = 0.94,
}: {
  onPress: () => void; style?: object; children: React.ReactNode;
  disabled?: boolean; scaleValue?: number;
}) {
  const scale = useSharedValue(1);
  const animStyle = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  return (
...
function SOSModal({
  ...
  return (
    <Modal transparent visible={visible} animationType="none">
      <ReAnimated.View style={[styles.modalOverlay, overlayStyle, { paddingBottom: 250 }]}>
...
// This replaces the always-visible bottom bar. A floating button on the map
// opens it; users can swipe down to dismiss.
function ActionSheetModal({
  ...
  return (
    <Modal transparent visible={visible} animationType="none" onRequestClose={onClose}>
      {/* Backdrop */}
      <Animated.View
        style={[StyleSheet.absoluteFillObject, { backgroundColor: "rgba(0,0,0,0.7)", opacity: backdropOpacity }]}
      >
      ...
      >
        {/* Drag handle — attached pan responder here */}
        ...
        >
          ...
          <View style={sheetStyles.metaRow}>
            <View style={sheetStyles.codeChip}>
              <Ionicons name="barcode-outline" size={13} color="rgba(255,255,255,0.5)" />
              <Text style={sheetStyles.codeText}>{trip?.trip_code ?? "—"}</Text>
            </View>
            <Text style={sheetStyles.durationText}>{formatDuration(elapsedSeconds)}</Text>
            {isDriver && (
              <View style={sheetStyles.earningsChip}>
                <Ionicons name="star" size={13} color={Colors.gold} />
                <Text style={sheetStyles.earningsChipText}>{formatCoins(earningsCoins)}</Text>
              </View>
            )}
          </View>
          ...
          {trip && (
            <View style={sheetStyles.routeRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={sheetStyles.routeStop} numberOfLines={1}>{trip.origin}</Text>
                <Text style={sheetStyles.routeStop} numberOfLines={1} >{trip.destination}</Text>
              </View>
            </View>
          )}
          ...
          {/* Driver: no passengers yet */}
          {isDriver && passengers.length === 0 && (
            <View style={sheetStyles.noPassRow}>
              <Ionicons name="person-add-outline" size={16} color="rgba(255,255,255,0.3)" />
              <Text style={sheetStyles.noPassText}>{t("trip.noPassengers")}</Text>
            </View>
          )}
...
function AIAssistantModal({
  ...
  const speakText = useCallback((id: string, text: string) => {
    ...
    setSpeakingId(id);
    Speech.speak(text, {
      language: "en-NG", rate: 1.05, pitch: 1.1, volume: 1.0,
  ...
  return (
    <Modal transparent visible={visible} animationType="none" onRequestClose={handleClose}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <ReAnimated.View style={[aiStyles.sheet, { paddingBottom: Math.max(insets.bottom, 16) }, sheetStyle]}>
          <View style={aiStyles.sheetHandle} />
          <View style={aiStyles.header}>
            <View style={aiStyles.headerLeft}>
          ...
          <ScrollView ref={scrollRef} style={aiStyles.chatScroll} contentContainerStyle={aiStyles.chatContent} showsVerticalScrollIndicator={false} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: true })}>
            {messages.map(msg => <ChatBubble key={msg.id} msg={msg} isSpeaking={speakingId === msg.id} onSpeak={text => { speakingId === msg.id ? stopSpeaking() : speakText(msg.id, text); }} />)}
            {loading && <View style={aiStyles.typingRow}><View style={aiStyles.typingBubble}><ActivityIndicator size="small" color={Colors.primary} /><Text style={aiStyles.typingText}>Teqil AI is thinking…</Text></View></View>}
          </ScrollView>
          <View style={aiStyles.inputRow}>
            <TextInput ref={inputRef} style={aiStyles.input} placeholder="Ask me anything about your trip…" placeholderTextColor="rgba(255,255,255,0.3)" value={input} onChangeText={setInput} multiline maxLength={200} returnKeyType="send" onSubmitEditing={() => sendMessage(input)} editable={!loading} />
            <Pressable style={[aiStyles.sendBtn, (!input.trim() || loading) && aiStyles.sendBtnDisabled]} onPress={() => sendMessage(input)} disabled={!input.trim() || loading}>
...
export default function LiveTripScreen() {
  ...
  useEffect(() => {
    ...
    const center = driverLocation
      ?? (originCoord && destCoord ? midpoint(originCoord, destCoord) : originCoord ?? destCoord);
    if (!center) return;
    if (routePoints.length > 1) {
  ...
  // Navigate to rating screen with full trip metadata
  const navigateToRating = useCallback((currentTrip: Trip, ratedUserId: string, role: "driver" | "passenger") => {
    router.replace({
      pathname: "/rating",
      params: {
        ...
        startTime: currentTrip.start_time,
        endTime: currentTrip.end_time ?? new Date().toISOString(),
      },
    });
  }, []);
  ...
  const handleEndTrip = useCallback(() => {
    ...
    Alert.alert(
      ...
      "This will complete the trip for all passengers.",
      [
        { text: t("common.cancel"), style: "cancel", onPress: () => setActionSheetVisible(true) },
        {
          text: t("trip.endTrip"),
          style: "destructive",
          onPress: async () => {
            ...
            if (trip) {
              await TripsStorage.update(trip.id, { status: "completed", end_time: endTime });
            }
            if (user) {
              syncAll({ id: user.id, role: user.role, park_name: user.park_name }).catch(() => {});
            }
            ...
            const finalTrip = { ...trip!, end_time: endTime, status: "completed" as const };
            await scheduleTripEndNotification({ trip: finalTrip, role: "driver" }).catch(() => {});

            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
            resetTripState();

            // Navigate to rating — driver rates the overall experience
            // We use a placeholder ratedUserId (first passenger) or skip rating
            if (passengers.length > 0) {
              navigateToRating(finalTrip, passengers[0].user_id, "driver");
            } else {
              router.replace("/(driver)");
            }
          },
  ...
  }, [trip, user, passengers, t, resetTripState, navigateToRating]);
      ...
      [
        { text: t("common.cancel"), style: "cancel", onPress: () => setActionSheetVisible(true) },
        {
          ...
          onPress: async () => {
            ...
            // Push notification to passenger + emergency contacts
            if (currentTrip) {
              const finalTrip = { ...currentTrip, end_time: endTime };
            ...
            // Navigate to rating — passenger rates driver
            if (currentTrip?.id && currentTrip?.driver_id) {
              const finalTrip = { ...currentTrip, end_time: endTime, status: "completed" as const };
              navigateToRating(finalTrip, currentTrip.driver_id, "passenger");
            } else {
              router.replace("/(passenger)");
            }
          },
  ...
  }, [displayTrip, myPassenger, user, t, resetTripState, navigateToRating]);
  ...
  const handleSOSConfirm = async () => {
    ...
    if (displayTrip) {
      await scheduleSOSNotification(displayTrip, contacts, locationStr).catch(() => {});
    }
    Alert.alert("SOS Sent", "Emergency contacts and park owner have been notified.");
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {originCoord && (
          <Marker coordinate={originCoord} title="Origin" anchor={{ x: 0.5, y: 0.5 }}>
            <View style={mapStyles.originMarker}><View style={mapStyles.originDot} /></View>
          </Marker>
        )}
        {destCoord && (
          <Marker coordinate={destCoord} title="Destination" anchor={{ x: 0.5, y: 1 }}>
            <View style={mapStyles.destMarkerWrap}>
              <View style={mapStyles.destMarker}><Ionicons name="flag" size={16} color="#fff" /></View>
              <View style={mapStyles.destMarkerTail} />
            </View>
        ...
        {passengerMarkers.map(({ passenger, coord }, i) => (
          <Marker key={passenger.id} coordinate={coord} anchor={{ x: 0.5, y: 1 }}>
            <View style={mapStyles.passengerMarkerWrap}>
      ...
      {/* Re-center */}
      <TouchableOpacity
        style={[styles.recenterBtn, { top: topPadding + (topBarCollapsed ? 64 : 120) }]}
      ...
      >
        ...
        <ReAnimated.View style={aiBtnStyle}>
          <AnimatedPressable
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setAiVisible(true); }}
            style={styles.aiFab}
            scaleValue={0.88}
          >
            <LinearGradient colors={["#2D2D2D", "#1A1A1A"]} style={styles.aiFabGradient}>
        ...
        {/* Trip info / action sheet toggle FAB */}
        <AnimatedPressable
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setActionSheetVisible(true); }}
...
const mapStyles = StyleSheet.create({
  ...
  destMarkerWrap: { alignItems: "center" },
  destMarker: {
    width: 34, height: 34, borderRadius: 10, backgroundColor: Colors.error,
    alignItems: "center", justifyContent: "center",
    shadowColor: Colors.error, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.5, shadowRadius: 6, elevation: 6,
  },
  destMarkerTail: {
    width: 0, height: 0,
    borderLeftWidth: 6, borderRightWidth: 6, borderTopWidth: 8,
    borderLeftColor: "transparent", borderRightColor: "transparent", borderTopColor: Colors.error,
    marginTop: -1,
  },
  driverMarker: { alignItems: "center", justifyContent: "center", position: "relative" },
  ...
  driverPulse: {
    ...
    borderWidth: 2, borderColor: Colors.primary + "55", backgroundColor: "transparent",
  },
  passengerMarkerWrap: { alignItems: "center" },
  passengerMarker: {
    width: 28, height: 28, borderRadius: 8, backgroundColor: Colors.error,
    alignItems: "center", justifyContent: "center",
    shadowColor: Colors.error, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.4, shadowRadius: 4, elevation: 4,
  },
  passengerMarkerText: { fontFamily: "Poppins_700Bold", fontSize: 12, color: "#fff" },
  passengerMarkerTail: {
    width: 0, height: 0,
...
const styles = StyleSheet.create({
  ...
  routeErrorBanner: {
    position: "absolute", bottom: 120, left: 16, right: 80,
    flexDirection: "row", alignItems: "center", gap: 6,
    backgroundColor: "rgba(20,30,22,0.9)", borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 8,
    borderWidth: 1, borderColor: "rgba(245,166,35,0.3)", zIndex: 10,
  },
  routeErrorText: {
    flex: 1, fontFamily: "Poppins_400Regular", fontSize: 11,
    color: "rgba(245,166,35,0.85)", lineHeight: 16,
  },
  ...
  topBarWrapper: {
    ...
    borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.07)", zIndex: 20,
  },
  topBarRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  topBarIconBtn: { width: 38, height: 38 },
  topBarIconInner: {
    flex: 1, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.1)",
    alignItems: "center", justifyContent: "center",
  },
  topBarCenter: { flex: 1, alignItems: "center" },
  topBarCenterContent: { flexDirection: "row", alignItems: "center", gap: 8 },
  ...
  topBarDetailRow: {
    ...
    borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.07)", flexWrap: "wrap",
  },
  topBarDetailItem: { flexDirection: "row", alignItems: "center", gap: 4, flex: 1, minWidth: 80 },
  topBarDetailText: { fontFamily: "Poppins_400Regular", fontSize: 11, color: "rgba(255,255,255,0.65)", flex: 1 },
  ...
  durationPill: {
    flexDirection: "row", alignItems: "center", gap: 5,
    backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 12, paddingHorizontal: 10, paddingVertical: 5,
  },
  durationText: { fontFamily: "Poppins_600SemiBold", fontSize: 12, color: "rgba(255,255,255,0.7)" },
  ...
  aiFab: {
    ...
    shadowColor: Colors.gold, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 12, elevation: 10,
  },
  aiFabGradient: { flex: 1, alignItems: "center", justifyContent: "center" },

  tripFab: {
    width: 72, height: 52, borderRadius: 26, overflow: "hidden",
    shadowColor: Colors.primary, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.5, shadowRadius: 14, elevation: 12,
  },
  tripFabGradient: {
  ...
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.72)", justifyContent: "flex-end" },
  sosSheet: {
    backgroundColor: "rgba(59,59,59,0.85)", borderRadius: 28, marginHorizontal: 12,
  ...
  sosIconRing: {
    ...
    marginBottom: 16, borderWidth: 2, borderColor: "rgba(239,68,68,0.3)",
  },
  sosTitle: { fontFamily: "Poppins_700Bold", fontSize: 20, color: "#fff", marginBottom: 8 },
  sosDesc: {
    fontFamily: "Poppins_400Regular", fontSize: 14, color: "rgba(255,255,255,0.55)",
...
const aiStyles = StyleSheet.create({
  ...
  quickChip: {
    ...
    borderWidth: 1, borderColor: "rgba(0,166,81,0.22)",
  },
  quickChipText: { fontFamily: "Poppins_500Medium", fontSize: 12, color: "rgba(255,255,255,0.8)" },
  chatScroll: { flex: 1, minHeight: 120 },
  chatContent: { paddingHorizontal: 16, paddingTop: 4, paddingBottom: 8, gap: 10 },
  ...
  bubbleUser: { alignSelf: "flex-end", backgroundColor: Colors.primary, borderBottomRightRadius: 4 },
  bubbleAI: {
    alignSelf: "flex-start", backgroundColor: "#1E2820",
    borderBottomLeftRadius: 4, borderWidth: 1, borderColor: "rgba(255,255,255,0.07)",
  },
  bubbleHeader: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 6 },
  aiBubbleIcon: {
  ...
  bubbleText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.85)", lineHeight: 20 },
  bubbleTextUser: { color: "#fff" },
  bubbleTime: { fontFamily: "Poppins_400Regular", fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 4, alignSelf: "flex-end" },
  typingRow: { alignSelf: "flex-start" },
  typingBubble: {
    flexDirection: "row", alignItems: "center", gap: 8,
    backgroundColor: "#1E2820", borderRadius: 16, borderBottomLeftRadius: 4,
  ...
  input: {
    flex: 1, minHeight: 42, maxHeight: 90,
    backgroundColor: "rgba(255,255,255,0.06)", borderRadius: 14,
    paddingHorizontal: 14, paddingVertical: 10,
    fontFamily: "Poppins_400Regular", fontSize: 13, color: "#fff",
