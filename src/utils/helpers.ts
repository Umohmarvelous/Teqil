...
// ─── ADD to src/utils/helpers.ts (replace generateDriverId and generateDriverIdFromUsername) ─
// These two functions replace the old implementations.
// The rest of helpers.ts is unchanged.

/**
 * Generate a canonical driver badge ID from a display name.
 *
 * Rule (spec §6):
 *   Take the first 6 alphabetic characters of the name, lowercase.
 *   "ChidiOkonkwo"  → "chidio"
 *   "Ada"           → "ada"      (fewer than 6 alpha chars → use what's there)
 *   "DJ Smoke 99"   → "djsmok"
 *
 * Uniqueness is enforced at the DB level (UNIQUE constraint on users.driver_id).
 * If the base ID is taken, callers should append a numeric suffix and retry.
 */
export function generateDriverIdFromUsername(name: string): string {
  // Strip everything except letters, then take first 6 lowercased
  const alpha = name.replace(/[^a-zA-Z]/g, "").toLowerCase();
  return alpha.slice(0, 6) || "driver";
}

/**
 * If the base ID is already taken, generate a fallback with a 2-digit suffix.
 * "chidio" → "chidio42"
 * Retry up to 10 times before giving up (caller catches the DB unique violation).
 */
export function generateDriverIdWithSuffix(name: string): string {
  const base   = generateDriverIdFromUsername(name);
  const suffix = Math.floor(10 + Math.random() * 89).toString(); // "10"–"99"
  return `${base}${suffix}`;
}

/**
 * Legacy alias kept for any callers that still use the old signature.
 * @deprecated Use generateDriverIdFromUsername
 */
export function generateDriverId(): string {
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  let suffix = "";
