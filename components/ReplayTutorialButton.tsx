...
export default function ReplayTutorialButton() {
  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }}>
      <Pressable style={[S.row, { backgroundColor: bg, borderColor: border }]} onPress={handlePress}>
        <View style={S.iconWrap}>
          <HugeiconsIcon icon={PlayIcon as any} size={20} color={Colors.primary} />
        </View>
        <View style={S.textBlock}>
          <Text style={[S.label, { color: textColor }]}>Replay app tutorial</Text>
          <Text style={[S.sub, { color: subColor }]}>Walk through the feature overview again</Text>
        </View>
        <HugeiconsIcon icon={ChevronRight as any} size={16} color={subColor} />
      </Pressable>
    </Animated.View>
...
const S = StyleSheet.create({
  row: { flexDirection: "row", alignItems: "center", gap: 14, borderRadius: 16, paddingHorizontal: 16, paddingVertical: 14, borderWidth: 1 },
  iconWrap: { width: 38, height: 38, borderRadius: 10, backgroundColor: "rgba(0,154,67,0.12)", alignItems: "center", justifyContent: "center" },
  textBlock: { flex: 1 },
  label: { fontFamily: "Poppins_500Medium", fontSize: 14 },
  sub: { fontFamily: "Poppins_400Regular", fontSize: 12, marginTop: 1, lineHeight: 16 },
});